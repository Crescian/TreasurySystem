        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-700">List of Vendors</h3>
...
<script>
            ...
            <div  x-data @click.prevent="$dispatch('open-modal', 'edit-company')" class="bg-gray-100 p-4 rounded-lg shadow-sm border border-gray-200" onclick="editCompany(${id});">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Industry: Vendor</p>
                    </div>
                </div>
                <div class="mt-3">
    ...
    function getCompanies() {
        $('.CompanyList').html('');
        $.ajax({
            url: @json(route('company.showVendor')), // Ensure proper URL formatting
            type: 'GET',
            dataType: 'json', // Specify JSON response type
            success: function(response) {
                if (response.length === 0) {
                    $('.CompanyList').html(
                        '<div class="alert alert-info">There are currently no vendors.</div>');
                    return;
                }
                $.each(response, (key, value) => {
