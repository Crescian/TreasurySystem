<x-app-layout>
    ...
    <x-modal name="edit-company" focusable>
        <div class="p-4">
            ...
            <div class="mt-3">
                <x-input-label for="company" :value="__('Company')" />
                <x-text-input id="editCompany" name="editCompany" type="text" class="mt-1 block w-full" disabled
                    required autofocus autocomplete="editCompany" />
            </div>
            ...
            <div class="mt-3">
                ...
                <x-text-input id="editAddress" name="editAddress" type="text" class="mt-1 block w-full" required autofocus
            ...
            <div  x-data @click.prevent="$dispatch('open-modal', 'edit-company')" class="bg-gray-100 p-4 rounded-lg shadow-sm border border-gray-200" onclick="editCompany(${id});">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">Industry: Company</p>
                    </div>
                </div>
                <div class="mt-3">
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.length === 0) {
                    $('.CompanyList').html(
                        '<div class="alert alert-info">There are currently no companies.</div>');
                    return;
                }
                $.each(response, (key, value) => {
